...
xrange = -197;
for i = 1:1
    Rotation   = rodriguesVectorToMatrix([0.0,0.0,0.1]' * i/10 );% + randn(3,1)*0.001);
    
    
    DepthInput = canonical_xyz (KnnIndex,:); %% * Rotation; % randn(KnnIndexLen,3) * 8 + 1  
    len = sum(DepthInput(:,1)< xrange);
    DepthInput(DepthInput(:,1)< xrange, :) = (DepthInput(DepthInput(:,1)< xrange, :) - repmat([xrange, 245, 20], len, 1))  *  Rotation + repmat([xrange,  245, 20], len, 1);
  
    DepthInputAll = canonical_xyz;%%  * Rotation; 
    len = sum(DepthInputAll(:,1)< xrange);
    DepthInputAll(DepthInputAll(:,1)< xrange, :) = (DepthInputAll(DepthInputAll(:,1)< xrange, :) - repmat([xrange, 245, 20], len, 1))  *  Rotation + repmat([xrange, 245, 20], len, 1);
